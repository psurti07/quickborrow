...
if(!function_exists('sendSingleSMS')){
    function sendSingleSMS($mobile, $otp, $panel = 'self'){
        $message = "Hello, the QuikBorrow OTP for your mobile number registration is ".$otp.". Kindly do not share it with anyone. Thanks, QuikBorrow";
...
if(!function_exists('sendDynamicSMS')){
    function sendDynamicSMS($senderId, $message, $mobile, $panel = 'assistant', $type = '', $tempId = ''){
        ...
        // Log::info("FP SMS response : " . $response);
        // Return the response
        return [
            'status_code' => $response->status(),
            'body' => $response->body(),
